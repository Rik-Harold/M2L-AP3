<template>
  <div id="documentCerfa">
    ...
    <div class="d-flex justify-content-center controls" style="margin-top: 10px;margin-bottom: 10px;">
        <button id="send_getPDF_cerfa" class="btn btn-primary" @click="docPDF(annee, id)">Télécharger et sauvegarder en PDF</button>
    </div>
    <div class="d-flex justify-content-center controls" style="margin-bottom: 50px;">
        <div class='btn btn-info all-bordereaux-button'><span><i class="bi bi-arrow-left-short"></i></span>
            <router-link to="/tresorier/les-bordereaux" class="accueil">Revenir à l'accueil</router-link>
        </div>
        <div class='btn btn-info all-bordereaux-button ms-3'><span><i class="bi bi-arrow-left-short"></i></span>Récapitulatif</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    docPDF: (annee, id) => {
      ...
      // Nom du bordereau
      const pdfName = 'document_cerfa_' + annee + '_' + id
      ...
      doc.html(documentCerfa, {
        callback: function (doc) {
          ...
          sendDocumentCerfa(doc.output('arraybuffer', documentCerfa))
            .then(res => res.json())
            .then(data => {
              if (data.ok === 'ok') {
                // Alerte de réception
